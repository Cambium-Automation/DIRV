...
This layout aggregates running counts, mean routing latencies, mathematical utility metrics, and activation deltas. This profile exposes zero-overhead local telemetry to external tools like cambiumctl or the native libcambium C API. The profile is a hashed behavioral record of routing decisions for data-in-use verification and governance; it is not a cryptographic proof. Bundle integrity belongs to SHA-256 checksums and, before customer distribution, signed manifests.
